...
from mgit.changesets.models import ChangesetCollection, RepositoryChangeset
from mgit.changesets.storage import ChangesetStorage, ChangesetStorageError
from mgit.pipeline.validation import (
    ChangesetValidator,
    ValidationResult,
    ValidationSeverity,
)
...
class ChangesetRecoveryManager:
    ...
    def __init__(
        self,
        storage: ChangesetStorage | None = None,
    ...
    def recover_changeset_operation(
        ...
        **operation_kwargs,
        ...
        for attempt in range(self.max_recovery_attempts):
            try:
                logger.debug(
                    f"Attempting operation (attempt {attempt + 1}/{self.max_recovery_attempts})"
                )

                # Validate changeset before operation
                validation_result = self.validator.validate_repository_changeset(
                    changeset
                )

                if (
                    not validation_result.is_valid
                    and strategy != RecoveryStrategy.IGNORE
                ):
                    # Attempt to repair validation issues
                    repair_result = self._repair_changeset_validation_issues(
                ...
                if attempt < self.max_recovery_attempts - 1:
                    ...
                    recovery_action = self._apply_recovery_strategy(
                        e, changeset, strategy
                    )
                    ...
                    if recovery_action.success:
                        ...
                        if (
                            hasattr(recovery_action, "recovered_data")
                            and recovery_action.data_context
                        ):
                            recovered_changeset = recovery_action.data_context.get(
                                "recovered_changeset"
                            )
    ...
    def recover_storage_operation(
        ...
        try:
            logger.info(
                f"Attempting storage recovery for collection: {collection_name}"
            )
            ...
            if backup_result.recovery_successful:
                ...
                repair_result = self._repair_corrupted_storage(
    ...
    def create_recovery_checkpoint(
        self, collection_name: str, data: Any
    ) -> RecoveryAction:
        """
        ...
        try:
            ...
            if isinstance(data, ChangesetCollection):
                self.storage.save_changeset_collection(
                    data, f"{collection_name}.checkpoint"
                )
            else:
    ...
    def _repair_changeset_validation_issues(
        self, changeset: RepositoryChangeset, validation_result: ValidationResult
    ) -> RecoveryResult:
        ...
        try:
            ...
            for issue in validation_result.issues:
                if issue.severity in {
                    ...
                    repair_action = self._repair_validation_issue(
                        recovered_changeset, issue
                    )
                    actions_taken.append(repair_action)

                    if repair_action.success and repair_action.data_context:
                        # Apply repair if successful
                        repaired_changeset = repair_action.data_context.get(
                            "repaired_changeset"
                        )
            ...
            validation_result_after_repair = (
                self.validator.validate_repository_changeset(recovered_changeset)
            )
    ...
    def _repair_validation_issue(
        self, changeset: RepositoryChangeset, issue
    ) -> RecoveryAction:
        ...
        try:
            if issue.code == "MISSING_REQUIRED_FIELD":
                ...
                if field_name == "timestamp":
                    ...
                    changeset.has_uncommitted_changes = (
                        len(changeset.uncommitted_files) > 0
                    )
    ...
    def _attempt_data_repair(
        self, error: Exception, changeset: RepositoryChangeset
    ) -> RecoveryAction:
    ...
    def _apply_fallback_strategy(
        ...
        try:
            ...
            return RecoveryResult(
                recovery_successful=False, actions_taken=actions_taken
            )
    ...
    def _recover_from_backup(self, collection_name: str) -> RecoveryResult:
        ...
        try:
            ...
            if backup_path.exists():
                collection = self.storage.load_changeset_collection(
                    f"{collection_name}.backup"
                )
                if collection:
                    actions_taken.append(
                        RecoveryAction(
        ...
        return RecoveryResult(recovery_successful=False, actions_taken=actions_taken)

    def _repair_corrupted_storage(
        self, collection_name: str, operation_type: str
    ) -> RecoveryResult:
...
def recover_changeset_operation(
    operation, changeset: RepositoryChangeset, **kwargs
) -> RecoveryResult:
...
def recover_storage_operation(
    collection_name: str, operation_type: str
) -> RecoveryResult:
