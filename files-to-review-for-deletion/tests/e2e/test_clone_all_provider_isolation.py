...
def test_clone_all_provider_isolation(tmp_path):
    ...
    for provider_type, provider_names in providers_by_type.items():
        ...
        if provider_type == "azuredevops":
            ...
            if provider_type == "bitbucket" and workspace in [
                "large-workspace",
                "example-networks",
            ]:
                print(
                    f"⚠️  Skipping {workspace} - known to have too many repos for timeout limit"
                )
                results[provider_type] = {
                    "status": "skipped",
                    "reason": "too many repos",
        ...
        code, stdout, stderr = run_mgit_command(
            ...
            timeout=30,
        )  # 30 second timeout to avoid long waits
    ...
    for clone_dir in clone_dirs:
        if clone_dir.is_dir():
            repo_count = len(
                [d for d in clone_dir.iterdir() if d.is_dir() and (d / ".git").exists()]
            )
    ...
    assert (
        successful > 0
    ), f"No provider successfully cloned actual repositories. Results: {results}"
    assert (
        total_repos_cloned > 0
    ), f"No repositories were cloned across all providers. Results: {results}"
    ...
    if successful > 1:
        ...
        assert len(set(success_dirs)) == len(
            success_dirs
        ), "Provider isolation violated"
