...
def test_clone_all_with_discovery(tmp_path):
    ...
    for provider_type, provider_names in providers_by_type.items():
        ...
        list_code, list_stdout, list_stderr = run_mgit_command(
            [
                ...
                provider_name,
                "--limit",
                "50",  # Get enough to find patterns
                "--format",
                "json",
            ]
        )
        ...
        try:
            ...
            if provider_type == "azuredevops":
                ...
                if repo_count > 50:
                    print(
                        f"⚠️  Too many repos ({repo_count}), skipping to avoid timeout"
                    )
                    results[provider_type] = {
                        "status": "skipped",
        ...
        code, stdout, stderr = run_mgit_command(
            [
                ...
                "--config",
                provider_name,  # Use --config instead of --provider
                "--concurrency",
                "2",  # Low concurrency to be gentle
                "--update-mode",
                "skip",
            ],
            timeout=180,
    ...
    assert (
        successful > 0
    ), f"No provider types could successfully clone with partial matching. Results: {results}"
...
def test_clone_all_specific_patterns(tmp_path):
    ...
    for pattern in patterns_to_try:
        ...
        code, stdout, stderr = run_mgit_command(
            ["list", query, "--provider", test_provider, "--limit", "5"]
        )
    ...
    if found_pattern:
        ...
        code, stdout, stderr = run_mgit_command(
            [
                ...
                test_provider,
                "--filter",
                found_pattern,  # If filter is supported
                "--concurrency",
                "2",
                "--update-mode",
                "skip",
        ...
        if code == 0:
            ...
            print(
                f"✅ Successfully cloned {len(git_repos)} repos matching '{found_pattern}'"
            )
